...
Navigate to the `src/fizzbuzz.c` file. You'll write your code in there. When you want to test it, type `make clean all` into your terminal (make sure you're doing this from the directory where the `Makefile` is located), which will run the `Makefile` and compile your program and, if the compilation was successful, run the tests for it as well. 